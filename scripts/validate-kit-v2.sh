...
check "Aucune mention de l'auteur inspirateur externe dans le kit (anti-leak D9)" "[ \"\$(grep -rEi 'c''ole.?medin|c''oleam00' .claude/ docs/ examples/ memory/ scripts/ $CLAUDE_MD README.md MEMORY.md STRUCTURE.md 2>/dev/null | wc -l)\" = \"0\" ]"
...
check "Pas de mention Dipler dans le kit (sauf CHANGELOG)" "! grep -ril 'dipler' .claude/ examples/ README.md $CLAUDE_MD 2>/dev/null | grep -v CHANGELOG | grep -q ."
...
check "Vocabulaire 'un/le skill' masculin partout" "! grep -ri 'une skill\\|la skill\\|cette skill' .claude/ README.md $CLAUDE_MD 2>/dev/null | grep -q ."
