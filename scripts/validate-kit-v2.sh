...
check "L /livrer création projet Vercel via vercel.com/new (Dashboard)" "grep -q 'vercel.com/new' .claude/skills/livrer/SKILL.md"
check "L /livrer marqueur git ls-remote (pas gh repo view)" "grep -q 'git ls-remote' .claude/skills/livrer/SKILL.md"
check "L /livrer marqueur ship:url (pas .vercel/project.json en marqueur)" "awk '/Étape 3\\.V\\.0/,/Étape 3\\.V\\.1/' .claude/skills/livrer/SKILL.md | grep -q 'ship:url'"
check "L /livrer auth check via gh api user conservé (automation)" "grep -q 'gh api user' .claude/skills/livrer/SKILL.md"
